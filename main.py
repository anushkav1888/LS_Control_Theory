...
x_arr = [0,0]                # the center of the ball form the pivot.
...
while run:
    ...
    theta, integral, prev_error = c.pid(x,target, integral, prev_error, theta)
    dx, x_arr= c.solve(target,x_arr, theta)
    x=x_arr[0]
    print("velocity=", x_arr[1])
